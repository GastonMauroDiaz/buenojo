...
class MultipleChoiceQuestionGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the MultipleChoiceQuestion entity")
        ...
        .repeat(2) {
            ...
            .get("/api/multipleChoiceQuestions")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            ...
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "question":"SAMPLE_TEXT", "interactionType":null, "exerciseId":"0", "source":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_multipleChoiceQuestion_url")))
            .pause(10)
            .repeat(5) {
